...
TYPED_TEST(ArgMaxLayerTest, TestSetupAxisNegativeIndexing) {
  ...
  EXPECT_EQ(this->blob_top_->shape(2), this->blob_bottom_->shape(2));
  EXPECT_EQ(this->blob_top_->shape(3), argmax_param->top_k());
}
...
TYPED_TEST(ArgMaxLayerTest, TestCPUTopK) {
  ...
  for (int i = 0; i < num; ++i) {
    ...
    for (int j = 0; j < this->top_k_; ++j) {
      // TODO(chuck): make sure this does not affect classification pipeline
      // blob_top_->data_at(i, 0, j, 0) or equivalently data_at(i, 0, 0, j, 0)
      // will fail.
      max_ind = this->blob_top_->data_at(i, 0, j, 0, 0);
      max_val = bottom_data[i * dim + max_ind];
      int count = 0;
      for (int k = 0; k < dim; ++k) {
...
TYPED_TEST(ArgMaxLayerTest, TestCPUMaxValTopK) {
  ...
  for (int i = 0; i < num; ++i) {
    ...
    for (int j = 0; j < this->top_k_; ++j) {
      max_ind = this->blob_top_->data_at(i, 0, j, 0, 0);
      max_val = this->blob_top_->data_at(i, 1, j, 0, 0);
      EXPECT_EQ(bottom_data[i * dim + max_ind], max_val);
      int count = 0;
      for (int k = 0; k < dim; ++k) {
        if (bottom_data[i * dim + k] > max_val) {
    ...
    for (int j = 0; j < shape[1]; ++j) {
      for (int k = 0; k < shape[4]; ++k) {
        for (int m = 0; m < this->top_k_; ++m) {
          max_ind = this->blob_top_->data_at(i, j, 0, m, k);
          max_val = this->blob_bottom_->data_at(i, j, 0, max_ind, k);
          EXPECT_GE(max_ind, 0);
          EXPECT_LE(max_ind, shape[3]);
          int count = 0;
          for (int l = 0; l < shape[3]; ++l) {
            if (this->blob_bottom_->data_at(i, j, 0, l, k) > max_val) {
              ++count;
            }
          }
          EXPECT_EQ(m, count);
  ...
  for (int i = 0; i < shape[0]; ++i) {
    for (int j = 0; j < shape[1]; ++j) {
      for (int k = 0; k < shape[2]; ++k) {
        for (int m = 0; m < this->top_k_; ++m) {
          max_val = this->blob_top_->data_at(i, j, k, 0, m);
          int count = 0;
          for (int l = 0; l < shape[3]; ++l) {
