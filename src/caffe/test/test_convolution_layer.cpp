...
TYPED_TEST(ConvolutionLayerTest, Test0DConvolution) {
  ...
  const int num = this->blob_top_->count(4);
  const int dim = this->blob_top_->shape(4);
  const int bottom_dim = this->blob_bottom_->shape(4);
  for (int n = 0; n < num; ++n) {
    for (int d = 0; d < dim; ++d) {
      weight_offset[0] = d;
...
TYPED_TEST(ConvolutionLayerTest, TestSimple3DConvolution) {
  ...
  bottom_shape[2] = 5;
  bottom_shape[3] = this->blob_bottom_vec_[0]->shape(3);
  bottom_shape[4] = this->blob_bottom_vec_[0]->shape(4);
  FillerParameter filler_param;
  GaussianFiller<Dtype> filler(filler_param);
  for (int i = 0; i < this->blob_bottom_vec_.size(); ++i) {
    this->blob_bottom_vec_[i]->Reshape(bottom_shape);
...
TYPED_TEST(ConvolutionLayerTest, TestNDAgainst2D) {
  ...
  bottom_shape[1] = 18;
  bottom_shape[2] = 1;
  bottom_shape[3] = kernel_h * 2;
  bottom_shape[4] = kernel_w * 2;
  FillerParameter filler_param;
  GaussianFiller<Dtype> filler(filler_param);
  for (int i = 0; i < this->blob_bottom_vec_.size(); ++i) {
    this->blob_bottom_vec_[i]->Reshape(bottom_shape);
