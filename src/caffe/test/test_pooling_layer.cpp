...
// ------------------------------------------------------------------------- //
// CudnnNdPoolingLayerTest
// ------------------------------------------------------------------------- //
template <typename Dtype>
class CudnnNdPoolingLayerTest : public GPUDeviceTest<Dtype> {
 protected:
  CudnnNdPoolingLayerTest()
      : blob_bottom_(new Blob<Dtype>()),
        blob_top_(new Blob<Dtype>()),
  ...
  virtual ~CudnnNdPoolingLayerTest() {
    delete blob_bottom_;
    delete blob_top_;
    delete blob_top_mask_;
  }
...
TYPED_TEST_CASE(CudnnNdPoolingLayerTest, TestDtypes);

TYPED_TEST(CudnnNdPoolingLayerTest, TestSetupCudnnNd) {
  LayerParameter layer_param;
  PoolingParameter* pooling_param = layer_param.mutable_pooling_param();
  pooling_param->mutable_kernel_shape()->add_dim(1);
  pooling_param->mutable_kernel_shape()->add_dim(3);
  ...
  pooling_param->mutable_stride_shape()->add_dim(2);
  pooling_param->mutable_stride_shape()->add_dim(2);
  CudnnNdPoolingLayer<TypeParam> layer(layer_param);
  layer.SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
  EXPECT_EQ(this->blob_top_->num(), this->blob_bottom_->num());
  EXPECT_EQ(this->blob_top_->channels(), this->blob_bottom_->channels());
  EXPECT_EQ(this->blob_top_->height(), 3);
...
TYPED_TEST(CudnnNdPoolingLayerTest, TestSetupPaddedCudnnNd) {
  LayerParameter layer_param;
  PoolingParameter* pooling_param = layer_param.mutable_pooling_param();
  pooling_param->mutable_kernel_shape()->add_dim(1);
  pooling_param->mutable_kernel_shape()->add_dim(3);
...
TYPED_TEST(CudnnNdPoolingLayerTest, TestForwardMaxCudnnNd) {
  this->TestForwardSquare();
  this->TestForwardRectHigh();
  this->TestForwardRectWide();
...
TYPED_TEST(CudnnNdPoolingLayerTest, TestGradientMaxCudnnNd) {
  for (int kernel_h = 3; kernel_h <= 4; kernel_h++) {
    for (int kernel_w = 3; kernel_w <= 4; kernel_w++) {
      ...
      PoolingParameter* pooling_param = layer_param.mutable_pooling_param();
      pooling_param->mutable_kernel_shape()->add_dim(1);
      pooling_param->mutable_kernel_shape()->add_dim(kernel_h);
      pooling_param->mutable_kernel_shape()->add_dim(kernel_w);
      pooling_param->mutable_stride_shape()->add_dim(1);
      pooling_param->mutable_stride_shape()->add_dim(2);
      pooling_param->mutable_stride_shape()->add_dim(2);
      pooling_param->mutable_pad_shape()->add_dim(0);
...
TYPED_TEST(CudnnNdPoolingLayerTest, TestForwardMaxPaddedCudnnNd) {
  LayerParameter layer_param;
  PoolingParameter* pooling_param = layer_param.mutable_pooling_param();
  pooling_param->mutable_kernel_shape()->add_dim(1);
  pooling_param->mutable_kernel_shape()->add_dim(3);
  ...
  pooling_param->mutable_stride_shape()->add_dim(2);
  pooling_param->mutable_pad_shape()->add_dim(0);
  pooling_param->mutable_pad_shape()->add_dim(2);
  pooling_param->mutable_pad_shape()->add_dim(2);
  pooling_param->set_pool(PoolingParameter_PoolMethod_MAX);
  this->blob_bottom_->Reshape(1, 1, 1, 3, 3);
  // Input:
  //     [ 1 2 4 ]
  //     [ 2 3 2 ]
  //     [ 4 2 1 ]
...
TYPED_TEST(CudnnNdPoolingLayerTest, TestForwardAveCudnnNd) {
  LayerParameter layer_param;
  PoolingParameter* pooling_param = layer_param.mutable_pooling_param();
  pooling_param->mutable_kernel_shape()->add_dim(1);
  pooling_param->mutable_kernel_shape()->add_dim(3);
  ...
  pooling_param->mutable_stride_shape()->add_dim(1);
  pooling_param->mutable_stride_shape()->add_dim(1);
  // Currently, CudnnNd pooling does not support padding, so we use
  // a simplified version of this test.
  pooling_param->mutable_pad_shape()->add_dim(0);
  pooling_param->mutable_pad_shape()->add_dim(0);
  pooling_param->mutable_pad_shape()->add_dim(0);
...
TYPED_TEST(CudnnNdPoolingLayerTest, TestGradientAveCudnnNd) {
  for (int kernel_h = 3; kernel_h <= 4; kernel_h++) {
    for (int kernel_w = 3; kernel_w <= 4; kernel_w++) {
      LayerParameter layer_param;
      PoolingParameter* pooling_param = layer_param.mutable_pooling_param();
...
TYPED_TEST(CudnnNdPoolingLayerTest, TestGradientAvePaddedCudnnNd) {
  for (int kernel_h = 3; kernel_h <= 4; kernel_h++) {
    for (int kernel_w = 3; kernel_w <= 4; kernel_w++) {
      LayerParameter layer_param;
      PoolingParameter* pooling_param = layer_param.mutable_pooling_param();
...
TYPED_TEST(CudnnNdPoolingLayerTest, TestSetupCudnnNd3d) {
  LayerParameter layer_param;
  PoolingParameter* pooling_param = layer_param.mutable_pooling_param();
  pooling_param->mutable_kernel_shape()->add_dim(1);
  pooling_param->mutable_kernel_shape()->add_dim(3);
  ...
  blob_shape[3]=6;
  blob_shape[4]=5;
  this->blob_bottom_->Reshape(blob_shape);
...
TYPED_TEST(CudnnNdPoolingLayerTest, TestSetupCudnnNd4d) {
  LayerParameter layer_param;
  PoolingParameter* pooling_param = layer_param.mutable_pooling_param();
  ...
  EXPECT_EQ(this->blob_top_->shape(5), 4);
}
